    ...
    with psycopg.connect(connection_string) as conn:
        ...
        with conn.cursor() as cur:
            ...
            for movie_data in movies_data:
                year_released = movie_data[1] if movie_data[1] == 'NULL' else f"'{movie_data[1]}-01-01'"
            ...
            users_set = set()
            user_str = "INSERT INTO netflix_user (id) VALUES "
            rating_str = "INSERT INTO netflix_rating (user_id, movie_id, rating, award_date) VALUES "

            for movie_id, ratings in ratings_data.items():
                for rating in ratings:
                    if rating["user_id"] not in users_set:
                        users_set.add(rating["user_id"])
                        user_str += f"( {rating['user_id']} ),"
                
                rating_str += f"( {rating['user_id']}, {movie_id}, {rating['rating']}, '{rating['date']}' ),"

            user_str = user_str[:-1] + ";"
            rating_str = rating_str[:-1] + ";"

            cur.execute(user_str)

            # Make the changes to the database persistent
            conn.commit()

            cur.execute(rating_str)
